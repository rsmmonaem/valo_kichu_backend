...
const EditProduct = () => {
    ...
    const fetchProductData = async () => {
        try {
            ...
            if (data.colors && Array.isArray(data.colors)) {
                const colorsData = data.colors.map(color => ({
                    id: color.id || color.name.toLowerCase().replace(/\s+/g, '-'),
                    name: color.name,
                    color: color.color_class || availableColors.find(c => c.name === color.name)?.color || "bg-gray-400",
                    image: color.image || ""
                }));
                setSelectedColors(colorsData);
            }

            // Set variations with proper price
            if (data.variations && Array.isArray(data.variations)) {
                const variationsData = data.variations.map((variation, index) => ({
                    id: variation.id || index + 1,
                    color: variation.color || "Custom",
                    colorClass: variation.colorClass || "bg-gray-400",
                    code: variation.code || "",
                    sku: variation.sku || "",
            ...
            if (data.attributes && Array.isArray(data.attributes)) {
                const attributesData = data.attributes.map(attr => ({
                    id: attr.id || availAttributes.find(a => a.name === attr.name)?.id || Date.now(),
                    name: attr.name,
                    value: attr.values || attr.value || []
                }));
    ...
    /* ================= VARIATION GENERATION - MATCHING ADD PRODUCT ================= */
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (url.includes(import.meta.env.VITE_API_BASE_URL)) {
                                            ...
                                            // For external URLs, store the full URL
